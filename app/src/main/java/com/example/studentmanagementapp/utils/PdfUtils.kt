...
object PdfUtils {
    ...
    fun generateAttendanceSessionPdf(
        context: Context,
        course: Course,
        sessionDate: Long,
        students: List<Student>,
        ...
        val document = PdfDocument()
        val formattedDate = SimpleDateFormat("MMM dd, yyyy", Locale.getDefault()).format(sessionDate)
        return try {
            val pageInfo = PdfDocument.PageInfo.Builder(595, 842, 1).create()
            val page = document.startPage(pageInfo)
            val canvas = page.canvas
            ...
            canvas.drawText("Course: ${course.courseName} (${course.courseCode})", 40f, yPos, textPaint)
            yPos += 20f
            canvas.drawText("Date: $formattedDate", 40f, yPos, textPaint)
            yPos += 20f
            canvas.drawText("Instructor: ${course.instructorName}", 40f, yPos, textPaint)
            yPos += 30f
            canvas.drawText("Student", 40f, yPos, titlePaint)
            ...
            if (!dir.exists()) dir.mkdirs()
            val file =
                File(dir, "attendance_${course.courseCode}_${formattedDate}_${System.currentTimeMillis()}.pdf")
            FileOutputStream(file).use { output ->
                document.writeTo(output)
            }
            file
